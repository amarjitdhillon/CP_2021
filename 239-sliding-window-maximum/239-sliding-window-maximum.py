...
class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        # we will maintain the queue in monotinic decreasing fashion, so that left value is the max one
        w = deque()
        res = []
        
        for i,x in enumerate(nums):
            # If the current val is greater then the right most element in queue, then remove it. 
